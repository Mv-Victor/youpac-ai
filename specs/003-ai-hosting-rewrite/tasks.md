...
- [ ] T002 重写 `convex/autopilot.ts`：实现 `enableAutopilot` public mutation（鉴权、images 非空校验、删除旧 job、创建新 AutopilotJob、调度 runAutopilotStep，幂等忽略已运行托管）
- [ ] T003 在 `convex/autopilot.ts` 实现 `disableAutopilot` public mutation（鉴权、取消 pendingScheduledJobId、删除 job 记录、写 autopilotEnabled=false）
- [ ] T004 [P] 在 `convex/autopilot.ts` 实现 `getProjectAutopilotStatus` public query（返回 `{ enabled, failed } | null`）
...
- [ ] T011 [US1] 在 `convex/autopilotActions.ts` 的 `runAutopilotStep` 中实现 `handleMediaUpload` handler（检查 images.length > 0 且所有 aiDescription 非空，调用 `internal.autopilot._completeMediaUpload` 或等效确认 mutation）
- [ ] T012 [US1] 在 `convex/autopilotActions.ts` 的 `runAutopilotStep` 中实现 `handleMemeRecall` handler（检查 memeRecall status === idle，调用 `_completeMemeRecall` internalMutation）
